...
public class cPT_XMLReader {
	...
	*/
	...
	public void Parse(string eS) {
		xmlText = eS;
		xml = new cPT_XMLHashtable();
		Parse(eS, xml);
	}
	
	// This function will parse a possible series of tags
	void Parse(string eS, cPT_XMLHashtable eH) {
		eS = eS.Trim();
		while(eS.Length > 0) {
			eS = ParseTag(eS, eH);
			eS = eS.Trim();
	...
	string ParseTag(string eS, cPT_XMLHashtable eH) {
		// search for "<"
		int ndx = eS.IndexOf("<");
		...
		if (!eH.ContainsKey(tag)) {
			eH[tag] = new cPT_XMLHashList();
		}
		// Create a hashtable to contain this tag's information
		cPT_XMLHashList arrL = eH[tag] as cPT_XMLHashList;
		//int thisHashIndex = arrL.Count;
		cPT_XMLHashtable thisHash = new cPT_XMLHashtable();
		arrL.Add(thisHash);
...
public class cPT_XMLHashList {
	public ArrayList list = new ArrayList();
	
	public cPT_XMLHashtable this[int s] {
		get {
			return(list[s] as cPT_XMLHashtable);
		}
		set {
			list[s] = value;
		}
	...
	public void Add(cPT_XMLHashtable eH) {
		list.Add(eH);
	}
...
public class cPT_XMLHashtable {
	
	public List<string>				keys = new List<string>();
	public List<cPT_XMLHashList>		nodesList = new List<cPT_XMLHashList>();
	public List<string>				attKeys = new List<string>();
	public List<string>				attributesList = new List<string>();
	
	public cPT_XMLHashList Get(string key) {
		int ndx = Index(key);
		if (ndx == -1) return(null);
		return( nodesList[ndx] );
	}
	
	public void Set(string key, cPT_XMLHashList val) {
		int ndx = Index(key);
		if (ndx != -1) {
			nodesList[ndx] = val;
		} else {
	...
	public cPT_XMLHashList this[string s] {
		get {
			return( Get(s) );
		}
		set {
